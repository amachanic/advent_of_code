...
DECLARE @num_nodes INT = (SELECT COUNT(*) FROM (SELECT raw1 FROM @distances UNION SELECT raw2 FROM @distances) AS x);
...
WHILE 1=1
BEGIN;
    IF 1 = (SELECT COUNT(DISTINCT grp) FROM @groups) AND @num_nodes = (SELECT COUNT(raw) FROM @groups)
    BEGIN;
        SELECT
            CONVERT(BIGINT, LEFT(@point1, CHARINDEX(',', @point1) - 1)) * 
            CONVERT(BIGINT, LEFT(@point2, CHARINDEX(',', @point2) - 1));
        BREAK;
    END;
